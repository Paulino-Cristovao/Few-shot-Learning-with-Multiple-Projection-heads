...
def main():
    ...
    accuracy_save.extend([args.latent,test_avg_acc1,test_avg_acc1_])
    np.save(str(args.latent)+'/'+args.save+'_'+str(args.latent)+'_'+'acc.npy',accuracy_save)


    save_checkpoint({
            'state_dict': model.state_dict(),
            'best_prec1': test_acc1_,
...
def imprint(novel_loader, model, select):
    ...
    for i in range(100):
        tmp = output_stack[target_stack == (i + 100)].mean(0) #if not args.random else torch.randn(int(args.latent))
