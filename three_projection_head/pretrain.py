...
from loss import CrossEntropyLoss, mixup, TripletLoss
...
def main():
    ...
    if not os.path.isdir(args.savefiles):
        mkdir_p(str(args.latent)+'/'+args.savefiles)
    ...
    optimizer = torch.optim.SGD([
            ...
            ], lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay) #, momentum=args.momentum
    ...
    if args.resume:
        ...
        print("=> loaded checkpoint '{}' (epoch {})"
                .format(str(args.latent)+'/'+args.resume, checkpoint['epoch']))
        logger = Logger(os.path.join(str(args.latent)+'/'+args.checkpoint, 'log.txt'), title=title, resume=True)
    else:
        logger = Logger(os.path.join(str(args.latent)+'/'+args.resume, 'log.txt'), title=title)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_loss, test_acc, conf_matrix, feature1, feature2, feature3 = validate(val_loader, model, criterion)
        ...
        }, is_best, checkpoint=str(args.latent)+'/'+args.checkpoint)
    ...
    # from fc1
    feature_fc1 = feature1[0].astype(np.float64)
    target_fc1 = feature1[1]
    ...
    plt.scatter(np.reshape(feature_fc1[:, 0], -1), np.reshape(feature_fc1[:, 1], -1), c=np.reshape(target_fc1,-1), cmap=plt.cm.get_cmap("jet", 100))
    plt.colorbar(ticks=range(100))
    plt.title('Validation: Latent Projection-fc1')
    plt.savefig(str(args.latent)+'/'+args.savefiles+'/fc1.png', bbox_inches='tight')
    #plt.show()
    plt.clf()

    # plot confusion matrix
    test_acc = str(round(test_acc, 2))
    plot_confusion_matric(conf_matrix,str(args.latent)+'/'+args.savefiles,'fc1',test_acc)
    ...
    plt.rcParams['figure.figsize'] = 20, 20
    plt.scatter(np.reshape(feature_fc2[:, 0], -1), np.reshape(feature_fc2[:, 1], -1), c=np.reshape(target_fc2,-1), cmap=plt.cm.get_cmap("jet", 100))
    plt.colorbar(ticks=range(100))
    plt.title('Validation: Latent Projection-fc2')
    plt.savefig(str(args.latent)+'/'+args.savefiles+'/fc2.png', bbox_inches='tight')
    #plt.show()
    plt.clf()
    ...
    # fc3
    feature_fc3 = feature3[0].astype(np.float64)
    target_fc3 = feature3[1]

    # feature fc2
    feature_fc3 = bh_sne(feature_fc3[-5000:])
    target_fc3 = target_fc3[-5000:]

    plt.rcParams['figure.figsize'] = 20, 20
    plt.scatter(np.reshape(feature_fc3[:, 0], -1), np.reshape(feature_fc3[:, 1], -1), c=np.reshape(target_fc3,-1), cmap=plt.cm.get_cmap("jet", 100))
    plt.colorbar(ticks=range(100))
    plt.title('Validation: Latent Projection-fc3')
    plt.savefig(str(args.latent)+'/'+args.savefiles+'/fc1.png', bbox_inches='tight')
    #plt.show()
    plt.clf()
    ...
    history.plot(str(args.latent)+'/'+args.savefiles)
...
def validate(val_loader, model, criterion):
    ...
    latent_proj1 = Proj_Latent()
    latent_proj2 = Proj_Latent()
    latent_proj3 = Proj_Latent()
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(val_loader):
            ...
            # compute output
            output,feature3, feature2, feature1 = model(input)
            ...
            data_feature_extractor1 = latent_proj1.append(feature1,label)
            data_feature_extractor2 = latent_proj2.append(feature2,label)
            data_feature_extractor3 = latent_proj3.append(feature3,label)
            ...
            bar.next()
        bar.finish()
    return (losses.avg, top1.avg, conf_matrix, data_feature_extractor1, data_feature_extractor2, data_feature_extractor3)
