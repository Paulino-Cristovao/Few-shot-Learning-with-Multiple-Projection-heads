...
parser.add_argument('--latent', '--latent-size', default=16, type=int,
                    metavar='N', help='latent size (default: 16)')
...
def main():
    ...
    cm_avg3 = np.zeros((m, m))
    cm_avg3_ = np.zeros((m, m))
    ...
    # Data loading code # fc2 before relu
    for i in tqdm(range(10)):
    ...
    # Data loading code # fc3 before relu
    for i in tqdm(range(10)):
        imprint(novel_loader(), model, 4)
        test_acc3, cm_test3, feature3, = validate(load_val(), model, 4)
        test_avg3.append(test_acc3)
        cm_avg3 = cm_avg3 + cm_test3
    
    print(" ")
    # Accuracy- Test data [0-9]
    test_avg_acc3 = np.array(test_avg3).mean()
    test_avg_acc3 = str(round(test_avg_acc3,2))
    c_matrix3 = np.rint((cm_avg3 / 10))
    ...
    plt.rcParams['figure.figsize'] = 20, 20
    plt.scatter(np.reshape(feature_fc3[:, 0], -1), np.reshape(feature_fc3[:, 1], -1), c=np.reshape(target_fc3,-1), cmap=plt.cm.get_cmap("jet", 200))
    plt.colorbar(ticks=range(200))
    plt.title('Testing: Latent Projection')
    plt.savefig(str(args.latent)+'/'+args.save +'/fc3.png', bbox_inches='tight')
    #plt.show()
    plt.clf()

    # plot confusion matrix
    plot_confusion_matric(c_matrix3, str(args.latent)+'/'+args.save, 'fc3_before_relu',test_avg_acc3)


    #####
    # WI from fc3 after relu
    model = models.Net(args.latent).to(device)
    ...
    # Data loading code # fc3 after relu
    for i in tqdm(range(10)):
        imprint(novel_loader(), model, 5)
        test_acc3_, cm_test3_,_ = validate(load_val(), model, 5)
        test_avg3_.append(test_acc3_)
        cm_avg3_ = cm_avg3_ + cm_test3_

    print(" ")
    # Accuracy- Test data [0-9]
    test_avg_acc3_ = np.array(test_avg3_).mean()
    test_avg_acc3_ = str(round(test_avg_acc3_))
    c_matrix3_ = np.rint((cm_avg3_ / 10))

    # plot confusion matrix
    plot_confusion_matric(c_matrix3_, str(args.latent)+'/'+args.save, 'fc3_after_relu',test_avg_acc3_)

    accuracy_save.extend([args.latent,test_avg_acc1,test_avg_acc1_,test_avg_acc2,test_avg_acc2_,test_avg_acc3,test_avg_acc3_])
    np.save(str(args.latent)+'/'+args.save+'_'+str(args.latent)+'_'+'acc.npy',accuracy_save)


    save_checkpoint({
            'state_dict': model.state_dict(),
            'best_prec1': test_acc3,
...
def imprint(novel_loader, model, select):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(novel_loader):
            ...
            # compute output
            feature5,feature4,feature3,feature2,feature1,feature0 = model.helper_extract(input)
            ...
            if select==4:    feature = feature4 # before fc3 relu
            if select==5:    feature = feature5 # after fc3 relu
...
def validate(val_loader_test, model, select):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_loader_test):
            ...
            if select==4:   output,feature = model.forward_wi_fc3(input)
            if select==5:   output,feature = model.forward_wi_fc3_(input)
