...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_loss, test_acc, conf_matrix, feature1, feature2 = validate(val_loader, model, criterion)
    ...
    plt.rcParams['figure.figsize'] = 20, 20
    plt.scatter(np.reshape(feature_fc2[:, 0], -1), np.reshape(feature_fc2[:, 1], -1), c=np.reshape(target_fc2,-1), cmap=plt.cm.get_cmap("jet", 30))
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(val_loader):
            ...
            bar.next()
        bar.finish()
    return (losses.avg, top1.avg, conf_matrix, data_feature_extractor1, data_feature_extractor2)
